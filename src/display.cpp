...
Display::Display(Config *conf) {
  ...
  window = SDL_CreateWindow("Chip-8 Emulator", SDL_WINDOWPOS_CENTERED,
                            ...
                            conf->window_height * conf->scale_factor, 0);
...
void Display::update_screen(const Config *config, const Chip8 &chip8) {
  SDL_Rect rect = {.x = 0,
                   .y = 0,
                   .w = (int)config->scale_factor,
                   .h = (int)config->scale_factor};
  ...
  for (size_t i = 0; i < sizeof display; i++) {
    ...
    if (display[i]) {
      ...
      if (pixel_color[i] != config->fg_color) {
        ...
        pixel_color[i] = color_lerp(pixel_color[i], config->fg_color,
      ...
      if (pixel_color[i] != config->bg_color) {
        ...
        pixel_color[i] = color_lerp(pixel_color[i], config->bg_color,
                                    config->color_lerp_rate);
      }
...
uint32_t Display::color_lerp(const uint32_t start_color,
                             const uint32_t end_color, const float t) {
